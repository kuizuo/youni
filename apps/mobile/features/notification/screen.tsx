import { YStack, XStack, Image, Text, Separator, View } from "@/ui"
import { trpc } from "@/utils/trpc"
import { Stack, useRouter } from "expo-router"

export const NotificationScreen = (): React.ReactNode => {
  ...
  return <>
    ...
    <YStack flex={1} backgroundColor={'$background'}>
      <XStack justifyContent="space-around" padding="$4" alignItems='center' gap='$2' >
        <YStack
          position="relative" flex={1} justifyContent="center" alignItems="center" gap="$2"
        ...
        >
          ...
          {
            data?.count.comment! > 0 && <Text position="absolute" top={0} right={20}>{data?.count.comment} </Text>
          }
        ...
        >
          ...
          {
            data?.count.follow! > 0 && <Text position="absolute" top={0} right={20}>{data?.count.follow} </Text>
          }
