...
export class FollowService {
  ...
  /** 获取用户正在关注的用户 ID 列表。 */
  async getFollowingIds(userId: string, dto: CursorDto) {
    const { cursor = '0', limit } = dto

    const ids: string[] = []

    const [startCursor, keys] = await this.redis.sscan(getRedisKey(`following:${userId}`), cursor, 'COUNT', limit)
    ids.push(...keys)
  ...
  /** 获取关注用户的粉丝 ID 列表。 */
  async getFollowerIds(userId: string, dto: CursorDto) {
    const { cursor = '0', limit } = dto
